...
def test_prefixes():
    data = read_csv(data_dir.joinpath("column header data labels units prefixes.csv"))
...
def test_row_header_datalabels():
    data = read_csv(data_dir.joinpath("row header data labels.csv"))
...
def test_nan():
    data = read_csv(data_dir.joinpath("single column with NaN Inf -Inf.csv"))
...
def test_singlerow_timestamps():
    ts = convert_timestamp(read_csv(data_dir.joinpath("single row timestamps.csv")))
...
def test_unequal_lengths():
    data = read_csv(data_dir.joinpath("unequal lengths.csv"))
